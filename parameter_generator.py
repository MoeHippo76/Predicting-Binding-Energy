...
def Monte_Carlo_events(N,bins = 0,vis = False,dimens = 10,directory = None):
    List = []
    ev = Event_Visualizer()
    for i in range(N):
        Y = random_params()
        List.append(Y)   
    if vis == True:    
        ev.plot_distro(directory,N,bins,List,dimens)
    return List 
'''
...
class Event_Visualizer:
    ...
    '''
    def plot_distro(self, directory, N,bins,Events,dimen = 1):
        for i in range(dimen):
            x = self.plot_distro_single(N,Events,i+1,bins)
            plt.hist(x,bins,label = str(i+1) + "th variable")
        plt.legend()
        plt.savefig(f"{directory}/distribution of {dimen} vars.png")
        plt.show()
    ...
    '''
    def Plot_correlation_graph(self,directory):
        N = 100000
        bins = 100
        ...
        plt.savefig(directory+"/Correlation_table_figure.png")
        plt.show()